...
export default function LoHi() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrent(prev => (prev + 1) % loHiImages.length);
    }, 5000);
  ...
  const handleKey = useCallback(
    (e: KeyboardEvent) => {
      if (e.key === 'ArrowRight') setCurrent(prev => (prev + 1) % loHiImages.length);
      if (e.key === 'ArrowLeft') setCurrent(prev => (prev - 1 + loHiImages.length) % loHiImages.length);
    },
    []
  );
  useEffect(() => {
  ...
  return (
    <main className="min-h-screen bg-white text-black font-sans">
      ...
      <section className="w-full h-[60vh] sm:h-[80vh] relative flex items-center justify-center overflow-hidden">
        {loHiImages.map((img, idx) => (
          <Image
            key={img}
            src={img}
            alt={`LoHi's Project image ${idx + 1}`}
            fill
        ...
        {/* Prev/Next buttons */}
        <button
          onClick={() => setCurrent(prev => (prev - 1 + loHiImages.length) % loHiImages.length)}
        ...
        <button
          onClick={() => setCurrent(prev => (prev + 1) % loHiImages.length)}
